    ...
    <div className="bg-white border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.08)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.12)] transition-shadow rounded-2xl p-7 mb-10">
      <div className="flex gap-3 items-center mb-5 text-gray-700">
        ...
        <p className="text-[16px] font-medium leading-tight">
          {event.venueOption === "online" ? t(lang, "online") : event.venue}
        </p>
      </div>
          ...
          {event.price > 0 ? (
            <>
              ...
              <p className="text-2xl font-bold">
                {event.price} <span className="text-[13px] font-medium text-gray-500 ml-1">{t(lang, "onwards")}</span>
              </p>
            </>
          ) : (
            <p className="text-2xl font-bold ">{t(lang, "free")}</p>
          )}
        </div>
        {event.isPast ? (
          <button 
          ...
          >
            {event.ticketButtonText || t(lang, "registerNow")}
          </Link>
        )}
      </div>
    </div>
